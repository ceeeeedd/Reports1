...
select b.queue_number,
	   ...
	   (select top 1 cast(datediff(minute,qs.lu_updated_datetime,(select top 1 aaa.lu_updated_datetime from ITWORKSDEV01.QueueingDB.dbo.queue_summary aaa where aaa.queue_number = c.queue_number
																									and aaa.status_id = 2)) %60 as float
																									) from ITWORKSDEV01.QueueingDB.dbo.queue_summary qs where qs.queue_number = c.queue_number
																									and qs.status_id = 1)as served_min,
	   
	   (select top 1 cast(datediff(minute,qs.lu_updated_datetime,(select top 1 aa.lu_updated_datetime from ITWORKSDEV01.QueueingDB.dbo.queue_summary aa where aa.queue_number = c.queue_number
																									and aa.status_id = 2)) /60 as numeric(4,0)
																									) from ITWORKSDEV01.QueueingDB.dbo.queue_summary qs where qs.queue_number = c.queue_number
																									and qs.status_id = 5) as served_hr_recalled ,
	   (select top 1 cast(datediff(minute,qs.lu_updated_datetime,(select top 1 aaa.lu_updated_datetime from ITWORKSDEV01.QueueingDB.dbo.queue_summary aaa where aaa.queue_number = c.queue_number
																									and aaa.status_id = 2)) %60 as float
																									) from ITWORKSDEV01.QueueingDB.dbo.queue_summary qs where qs.queue_number = c.queue_number
																									and qs.status_id = 5)as served_min_recalled,
	   ...
	   (select top 1 cast(datediff(minute,qs.lu_updated_datetime,(select top 1 aaa.lu_updated_datetime from ITWORKSDEV01.QueueingDB.dbo.queue_summary aaa where aaa.queue_number = c.queue_number
																									and aaa.status_id = 3)) %60 as float
																									) from ITWORKSDEV01.QueueingDB.dbo.queue_summary qs where qs.queue_number = c.queue_number
																									and qs.status_id = 1)as missedcall_min,
	   ...
	   (select top 1 cast(datediff(minute,qs.lu_updated_datetime,(select top 1 aaa.lu_updated_datetime from ITWORKSDEV01.QueueingDB.dbo.queue_summary aaa where aaa.queue_number = c.queue_number
																									and aaa.status_id = 5)) %60 as float
																									) from ITWORKSDEV01.QueueingDB.dbo.queue_summary qs where qs.queue_number = c.queue_number
																									and qs.status_id = 3)as recalled_min,
	   (select top 1 cast(datediff(minute,qs.lu_updated_datetime,(select top 1 aa.lu_updated_datetime from ITWORKSDEV01.QueueingDB.dbo.queue_summary aa where aa.queue_number = c.queue_number
																									and aa.status_id = 4)) /60 as numeric(4,0)
																									) from ITWORKSDEV01.QueueingDB.dbo.queue_summary qs where qs.queue_number = c.queue_number
																									and qs.status_id = 2) as processed_hr ,
	   (select top 1 cast(datediff(minute,qs.lu_updated_datetime,(select top 1 aaa.lu_updated_datetime from ITWORKSDEV01.QueueingDB.dbo.queue_summary aaa where aaa.queue_number = c.queue_number
																									and aaa.status_id = 4)) %60 as float
																									) from ITWORKSDEV01.QueueingDB.dbo.queue_summary qs where qs.queue_number = c.queue_number
																									and qs.status_id = 2)as processed_min,
